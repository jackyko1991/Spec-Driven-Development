...
本文件介紹**規格驅動開發 (Specification-Driven Development, SDD)** 工作流程，這是一種使用 Claude Code、AWS Kiro 和 Roo Code 等代理式開發工具來建構軟體的結構化方法。SDD 將開發過程劃分為五個明確階段：**指導架構師**、**規劃**、**執行**、**測試**與**偵錯**。這種開發設計促進快速、高品質程式碼生成，並提升清晰度、可維護性與可追溯性。
...
3.  **⚡ 執行模式 (Execution Mode)**：以原子方式實現任務，驗證正確性，並增量更新程式碼庫與文件。
4.  **🧪 測試模式 (Tester Mode)**：在功能實現後編寫並運行測試以驗證其正確性。
5.  **🐞 偵錯模式 (Debugger Mode)**：一個結構化的工作流程，用於在隔離的沙箱環境中識別、複製和修復錯誤。
...
**職責：**
- 從 `.ai-rules/tech.md` 或 `specs/<feature-name>/.ai_rules/tech.md` 讀取測試策略。
- 根據 `requirements.md` 編寫並執行單元測試。
- 報告測試結果。

**工作流程：**
1.  **編寫測試**：實現涵蓋功能驗收標準的測試。
2.  **運行測試**：執行測試套件。
3.  **交接**：
    - 如果測試通過，建議返回 **執行模式** 繼續下一個任務。
    - 如果測試失敗，建議切換到 **偵錯模式** 解決問題。

---

## 5. 🐞 偵錯模式 (Debugger Mode)
